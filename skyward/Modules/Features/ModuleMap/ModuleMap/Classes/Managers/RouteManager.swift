...
class RouteManager {
    ...
    func cancelRecord() {
        guard let currentRouteId = currentRouteId else { return }
        DBManager.shared.deleteFromDb(fromTable: DBTableName.route.rawValue, where: RouteRecord.Properties.routeId == currentRouteId)
        DBManager.shared.deleteFromDb(fromTable: DBTableName.routePoint.rawValue, where: RoutePoint.Properties.routeId == currentRouteId)
        closeRecord()
    }
    ...
    func saveRoute(name: String, desc: String?) {
        ...
        guard var record = DBManager.shared.queryFromDb(fromTable: DBTableName.route.rawValue, cls: RouteRecord.self, where: RouteRecord.Properties.routeId == routeId)?.first else {
            return
        }
        // 更新路线基本信息
        record.name = name
        ...
        // 查询轨迹点
        guard let points = getPointsInRoute(routeId: routeId), !points.isEmpty else {
    ...
    func generateGPXFile(from points: [RoutePoint], outputURL: URL, name: String = "Generated Record") -> Bool {
        ...
        do {
            ...
            debugPrint("GPX文件生成成功：\(outputURL.path)")
            return true
        } catch {
