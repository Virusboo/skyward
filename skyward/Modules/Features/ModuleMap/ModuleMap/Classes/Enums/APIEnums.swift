...
enum MapAPI {
    ...
    case routePlan(_ request: RoutePlanRequest)                           // 路线规划
}
...
extension MapAPI: NetworkAPI {
    ...
    public var path: String {
        ...
        case .routePlan(let request):
            // 构建路线规划路径：/route/v1/{mode}/{coordinates}
            let coordsString = request.coordinates
                .map { "\($0.longitude),\($0.latitude)" }
                .joined(separator: ";")
            return "/route/v1/\(request.mode.path)/\(coordsString)"
        }
    }
