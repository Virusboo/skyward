...
public class POIDownloadManager {
    ...
    private func checkAndResetStaleStates() {
        ...
        // 检查时间间隔，重置已下载状态
        if downloadState == .downloaded, let lastTime = lastDownloadTime {
            let timeInterval = Date().timeIntervalSince(lastTime)
            if timeInterval >= Config.timeIntervalToReset {
                print("下载已超过24小时，重置状态")
                downloadState = .notDownloaded
                importState = .notImported
                downloadTaskInfo = nil
                importTaskInfo = nil
    ...
    private func cleanupFailedDownload() {
        // 删除临时文件
        if let taskInfo = downloadTaskInfo, let localPath = taskInfo.localFilePath {
                ...
                do {
                    ...
                    self.downloadFile(from: exportData.fileUrl, md5: exportData.fileMd5, version: exportData.version) { downloadResult in
                        ...
                        case .success(let localURL):
                            ...
                            updatedTaskInfo.localFilePath = localURL.path
                            updatedTaskInfo.downloadEndTime = Date()
                            self.downloadTaskInfo = updatedTaskInfo
        ...
        let task = URLSession.shared.downloadTask(with: url) { [weak self] localURL, response, error in
            ...
            do {
                ...
                let documentsURL = try self.fileManager.url(for: .documentDirectory,
                                                            ...
                                                            appropriateFor: nil,
                                                            create: true)
                let destinationURL = documentsURL.appendingPathComponent("poi_data_\(version).txt")
    ...
    private func startImportProcess(fileURL: URL, exportData: POIExportData, completion: ((Bool) -> Void)? = nil) {
        // 检查导入状态
        if importState == .importing, let taskInfo = importTaskInfo {
    ...
    private func continueImport(completion: ((Bool) -> Void)? = nil) {
        ...
        operationQueue.async { [weak self] in
            ...
            do {
                ...
                print("继续导入，从第 \(startLine) 行开始，总行数: \(lines.count)")
